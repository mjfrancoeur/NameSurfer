...
public class NameSurferGraph extends GCanvas
	...
	*/
	public NameSurferGraph() {
		...
		entryCompound = new GCompound(); // create entryCompound GCompound, containing entry GLines and GLabels
	...
	*/
	public void clear() {
		...
		entryCompound.removeAll(); // remove all GObjects from GCompound entryCompound
	...
	*/
	...
	for(int j = 0; j < keyOrder.size(); j++) {	
		...
		// In the order input by user, getting the name and array of graph points //
		
		key = keyOrder.get(j);
		rankArray = entryHashMap.get(key);
		
		for(int i = 1; i < rankArray.length; i++) {
			// Loop through rankArray (points to graph each name) plotting on the grid //
	...
	/* Method: setColor */
	/**
	 * Takes in two parameters, an int j and a GObject object and sets the object color
	 * alternating between four colors, based upon the value of the int j
	 * @param j
	 * @param object
	 */
	...
	*/
	public void update() {
		...
		// reset canvas height and width to be dependent upon current size of canvas //
